...
if __name__ == '__main__':
    while True:
        ...
        for task in tasks:
            print(datetime.datetime.now().strftime("%d/%m/%Y-%H:%M:%S.%f"), 'Task: {}'.format(task.id_))
            assing = client()
            ...
            if 'uuid' in send_document_response:
                ...
                worker.task_error(task.id_, '500', send_document_response['message'])
                continue
            ...
            webhook_url = 'http://' + DOMAIN_NAME + ':5000/api/v1/task_update?token=' + token
            
            print(datetime.datetime.now().strftime("%d/%m/%Y-%H:%M:%S.%f"), webhook_url)
